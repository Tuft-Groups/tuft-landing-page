...
import { FEATURE_DATA, type Feature, type Badge } from "@/lib/constants";
...
export function DetailedFeatures() {
  ...
  const activeFeature = FEATURE_DATA.find((f) => f.id === activeId);
    ...
    <section id="features" ref={containerRef} className="max-w-[1400px] mx-auto px-6 md:px-12 py-10 md:py-24 relative">
      <div className="flex flex-col md:flex-row gap-16 md:gap-32">
        ...
        <div className="w-full md:w-1/2 space-y-[45vh] md:space-y-[65vh] pb-[20vh] md:pb-[30vh] relative pl-10 md:pl-0">
          <div className="hidden lg:block absolute left-[-40px] md:left-[-60px] top-0 h-full w-[1px] bg-white/5">
            <motion.div
              style={{ top: progressTop, y: "-50%" }}
              className={cn(
                "absolute left-[-1px] w-[3px] h-[180px] rounded-full transition-[background,box-shadow] duration-500",
          ...
          <div className="sticky top-0 h-screen flex items-center justify-center perspective-[2500px]">
            ...
            <div className="w-[320px] lg:w-[360px] h-[650px] lg:h-[740px] relative transform-style-3d transition-transform duration-100 ease-out">
              <div className="absolute inset-0 rounded-[44px] overflow-hidden bg-black shadow-[0_0_0_2px_#333,0_0_0_4px_#1a1a1a,0_50px_100px_-20px_rgba(0,0,0,0.8)]">
                ...
                <img src={FEATURE_DATA.find((f) => f.id === activeId)?.mockup} className="w-full h-full object-cover" alt="Mockup Screen" />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div ref={ref} className={cn("reveal", isActive && "active")}>
      <div className="mb-10">
        ...
        >
          <span className={cn("w-8 h-[1px]", feature.color === "tuft-purple" ? "bg-tuft-purple" : "bg-tuft-pink")} />
          {feature.tag}
        </span>
      ...
      <div className="space-y-4 max-w-md">
        {feature.badges.map((badge: Badge, idx: number) => (
          <div
            key={idx}
            className="flex items-center gap-5 p-4 rounded-3xl bg-white/5 border border-white/10 tap-active cursor-pointer group hover:bg-white/[0.08] hover:border-white/20 transition-all duration-300 shadow-xl"
          >
            <div
              className={cn(
                "w-14 h-14 rounded-2xl bg-white/5  flex items-center justify-center transition-all shadow-2xl group-hover:scale-110 flex-shrink-0",
